...
public class DummyImages {
    ...
    private static final int[] widths = {1, 3, 4, 16, 9};
    private static final int[] heights = {1, 4, 3, 9, 16};

    public static List<ImagePOJO> getDummyList() {
        List<ImagePOJO> list = new ArrayList<>();
        for (String image : images) {
            int pos = RANDOM.nextInt(widths.length);
            list.add(new ImagePOJO(widths[pos], heights[pos], image));
        }
        return list;
    }
}
